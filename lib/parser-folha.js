...
/** Lê o PDF devolvendo, por página, as linhas de texto já ordenadas. */
async function lerLinhas(buffer) {
    ...
    for (let n = 1; n <= doc.numPages; n++) {
      ...
      paginas.push({
        ...
        linhas: [...porAltura.entries()]
          .sort((a, b) => b[0] - a[0])
          .map(([, pecas]) => pecas.sort((a, b) => a.x - b.x).map((p) => p.texto).join(' ').trim()),
      });
      pagina.cleanup();
    }
  } finally {
...
async function processarRelatorioFolha(buffer) {
  const paginas = await lerLinhas(buffer);
...
module.exports = { processarRelatorioFolha, ehRelatorioFolha, lerLinhas };
